...
void CambotImgProcessor::process()
{
    ...
    if ( cv_img_ptr_in != nullptr ) {
		...
		if(box_kalman.x > 0) {
    		cv::rectangle(cv_img_out.image, box_kalman, cv::Scalar(255, 0,0), 1);
		}
   	}
...
{
	try {
		box_detector.x = detect_msg.x;
		box_detector.y = detect_msg.y;
...
{
	try {
		box_kalman.x = kalman_msg.x;
		box_kalman.y = kalman_msg.y;
		lastCallback = ros::Time::now().toSec();
	} catch (ros::Exception& e) {
